...
function formatLeverage(leverage: MandateProfile["leverage"]): string {
  if (typeof leverage === "number") {
    return leverage <= 1 ? "无杠杆" : `${leverage}× leverage`;
  }
 ...
 * never `api.sendMessage`. "调整" sends a natural-language message back to the agent
...
export const MandateProposalCard = memo(function MandateProposalCard({ proposal, committed, onAdjust }: Props) {
  ...
  const handleCommit = useCallback(
    async (ordinal: number) => {
      ...
      if (!broker) {
        toast.error("无法提交授权令：缺少连接器券商信息。请让智能体重新生成方案。");
        return;
      }
      ...
      try {
        ...
        setBusyOrdinal(null);
        toast.error(error instanceof Error ? error.message : "提交授权令失败。");
      }
    },
